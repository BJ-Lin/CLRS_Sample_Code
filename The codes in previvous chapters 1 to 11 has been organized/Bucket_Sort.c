 ...
 *
 *
 * Time Complexity:  Theta( n + k ) + Theta( n^2 ) = Theta( n^2 + k ) = Theta( n^2 )																					  (Worst case ; n is the number of elements in Array, k is the number of buckets, and k is multiple of n, for instance, k = n , 2n, .. etc. ; All elements fall into a single bucket.)
 *					 Theta( n + k ) + Theta( k ) = Theta( n + k ) = Theta( n )																							  (Best case ; n is the number of elements in Array, k is the number of buckets, and k is multiple of n, for instance, k = n , 2n, .. etc. ; All elements fall into different buckets.)
 *					 Theta( n + k ) + k * Theta( ( n / k )^2 + ( n / k )( 1 - 1 / k ) ) = Theta( n + k ) + Theta( n^2 / k + n ) = Theta( n^2 / k + n + k ) = Theta( n )   (Average case ; n is the number of elements in Array, k is the number of buckets, and k is multiple of n, for instance, k = n , 2n, .. etc. ; The elements are uniformly and independently distributed into buckets. The derivation of time complexity is similar to the proof on CLRS text book p217. The time complexity is maximum expected value with maximum time cost of insertion sort, and the other time complexity of minimum expected value not showed up here is also Theta( n ).)
 * 
 * 
